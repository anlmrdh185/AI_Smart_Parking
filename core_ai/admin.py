...
if not st.session_state.admin_logged_in:
    ...
    if st.button("Secure Login"):
        # FYP Simple Auth (In production, use database hashed passwords)
...
if menu_selection == "🔍 Parking Monitoring":
    ...
    # --- CCTV STREAM VIEW ---
        ...
        with c1:
            ...
            # Mocking camera selection based on wings
...
# --- 6. PAGE: SETTINGS & MANAGEMENT ---
    ...
    with col1:
        ...
        # We use session state to mock saving the settings
        if 'fee_base' not in st.session_state: st.session_state.fee_base = 5.00
        if 'fee_add' not in st.session_state: st.session_state.fee_add = 3.00
        if 'fee_max' not in st.session_state: st.session_state.fee_max = 50.00
        
        base_fee = st.number_input("First Hour Rate (RM)", value=st.session_state.fee_base, step=1.0)
        st.caption("Base rate for the first hour of parking")
        
        add_fee = st.number_input("Additional Hour Rate (RM)", value=st.session_state.fee_add, step=1.0)
        st.caption("Rate for each additional hour after the first")
        
        max_fee = st.number_input("Maximum Daily Rate (RM)", value=st.session_state.fee_max, step=1.0)
        st.caption("Maximum amount charged per 24-hour period")
        
        if st.button("💾 Save Fee Structure", key="save_fees"):
            st.session_state.fee_base = base_fee
            st.session_state.fee_add = add_fee
            st.session_state.fee_max = max_fee
            st.success("Fee structure successfully updated!")
        ...
        <div style='background: #faf5ff; padding: 15px; border-radius: 8px; margin-top: 20px; border: 1px solid #e9d5ff;'>
            ...
            <span style='color: #6b7280; font-size: 14px;'>1 hour: </span> <span style='float:right; font-weight:bold;'>RM {base_fee:.2f}</span><br>
            <span style='color: #6b7280; font-size: 14px;'>3 hours: </span> <span style='float:right; font-weight:bold;'>RM {base_fee + (add_fee*2):.2f}</span><br>
            <span style='color: #6b7280; font-size: 14px;'>24 hours (max): </span> <span style='float:right; font-weight:bold;'>RM {max_fee:.2f}</span>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
